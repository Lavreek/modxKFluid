...
if (isset($modx->resourceIdentifier)) {
    ...
    $selectCategories = "SELECT `id`, `pagetitle` FROM `modx_site_content` WHERE `parent` = '$pageId' AND `published` = '1' AND `template` = '4'";
    ...
    if (!is_bool($categories)) {
        ...
        foreach ($categories as $category) {
            ...
            $selectChilds = "SELECT `id`, `pagetitle` FROM `modx_site_content` WHERE `template` = '2' AND `parent` = '{$category['id']}' AND `published` = '1'";
